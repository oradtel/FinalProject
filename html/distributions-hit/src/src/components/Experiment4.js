...
const Experiment4 = (props) => {
    ...
    useEffect(() => {
        (async () => {
            ...
            //console.log(10, data)
            let rowFromService1 = await getMyRow(3, 0, data);
            let rowFromService2 = await getMyRow(3, 5, data);
    ...
    const onSubmit = (data) => {
        var choice;
        if (isPicked1.picked){
            choice={choicePicked4:ID1,choiceNotPicked4:ID2};
        }else{
            choice={choicePicked4:ID2,choiceNotPicked4:ID1};
        }
        props.updateUser(choice);
        props.history.push('/experiment5');
    };
